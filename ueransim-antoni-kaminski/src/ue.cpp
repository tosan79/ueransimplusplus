...
static nr::ue::UeConfig *ReadConfigYaml()
{
    ...
    yaml::AssertHasField(config, "measurments");
    for (auto &meas : yaml::GetSequence(config, "measurments")) {
        RSSI rssi_slice{};
        int slice_cell_id = yaml::GetInt32(meas, "cell_nci");
        rssi_slice.RSRP = yaml::GetInt32(meas, "RSRP");
        rssi_slice.RSRQ = yaml::GetInt32(meas, "RSRQ");
        result->rssi[slice_cell_id] = rssi_slice;
    }
