...
void UeCmdHandler::handleCmdImpl(NmUeCliCommand &msg)
{
    ...
    case app::UeCliCommand::COVERAGE: {
        ...
        for (auto &item : cells)
        {
            ...
            RSSI rssi = m_base->config->rssi[cell.sib1.nci];
            auto signal = Json::Obj({
                {"RSRP", std::to_string(rssi.RSRP - 156.0) + " dBm"},
                {"RSRQ", std::to_string((rssi.RSRQ - 87.0) / 2.0 ) + " dB"},
            });
            ...
            if (cell.sib1.hasSib1)
            {
                sib1 = Json::Obj({
                    ...
                    {"q_RxLevMin", cell.sib1.sib1conf.q_RxLevMin * 2},
                    {"q_RxLevMinOffset", *cell.sib1.sib1conf.q_RxLevMinOffset},
                    {"q_RxLevMinSUL", *cell.sib1.sib1conf.q_RxLevMinSUL},
                    {"q_QualMin", *cell.sib1.sib1conf.q_QualMin},
                    {"q_QualMinOffset", *cell.sib1.sib1conf.q_QualMinOffset},
                });
            }

            if (cell.sib2.hasSib2) {
                sib2 = Json::Obj({
                    {"q_Hyst", cell.sib2.sib2conf.q_Hyst},
                    {"treshServingLowP", cell.sib2.sib2conf.treshServingLowP},
                    {"cellReselectionPriority", cell.sib2.sib2conf.cellReselectionPriority},
                    {"q_RxLevMin", cell.sib2.sib2conf.q_RxLevMin * 2},
                    {"s_IntraSearchP", cell.sib2.sib2conf.s_IntraSearchP * 2},
                    {"s_IntraSearchQ", *cell.sib2.sib2conf.s_IntraSearchQ},
                    {"t_ReselectionNR", cell.sib2.sib2conf.t_ReselectionNR},
                    {"deriveSSB_IndexFromCell", cell.sib2.sib2conf.deriveSSB_IndexFromCell},
                });
            }

            auto obj = Json::Obj({{"signal", signal},
                                  {"mib", mib},
                                  {"sib1", sib1},
                                  {"sib2", sib2}});
    ...
    case app::UeCliCommand::REGISTER: {
        Json json = Json::Obj({});
        m_base->nasTask->mm->auto_attach_blocked = false;
        
        sendResult(msg.address, json.dumpYaml());
        break;
    }
    case app::UeCliCommand::SET_RSRP: {
        Json json = Json::Obj({});
        auto curr_cell = m_base->shCtx.currentCell.get().cellId;
        m_base->config->rssi[curr_cell].RSRP = msg.cmd->x;

        sendResult(msg.address, json.dumpYaml());
        break;
    }
    case app::UeCliCommand::SET_RSRQ: {
        Json json = Json::Obj({});
        auto curr_cell = m_base->shCtx.currentCell.get().cellId;
        m_base->config->rssi[curr_cell].RSRQ = msg.cmd->x;
