...
void UeRrcTask::performCellReselection() {
    
    ActiveCellInfo cellInfo = m_base->shCtx.currentCell.get();
    int lastCellId = cellInfo.cellId;

    UeCellDesc cell_desc_s = m_cellDesc[cellInfo.cellId];
    RSSI rssi_s = m_base->config->rssi[cell_desc_s.sib1.nci];

    // intra-frequency reselection measurments condition:
    // Srxlev > sIntraSearchP && Squal > sIntraSearchQ
    // if true then do not perform measurments (and reselection)
    double s_RxLev = rssi_s.RSRP - 156.0 - (cell_desc_s.sib1.sib1conf.q_RxLevMin * 2.0 + *cell_desc_s.sib1.sib1conf.q_RxLevMinOffset);
    double s_Qual = (rssi_s.RSRQ - 87.0) / 2.0 - (*cell_desc_s.sib1.sib1conf.q_QualMin + *cell_desc_s.sib1.sib1conf.q_QualMinOffset);

    if (s_RxLev > cell_desc_s.sib2.sib2conf.s_IntraSearchP * 2.0 && s_Qual > *cell_desc_s.sib2.sib2conf.s_IntraSearchQ)
        return;

    // serving cell power:
    double q_Meas_s = rssi_s.RSRP - 156.0;

    // serving cell rank:
    double R_s = q_Meas_s + cell_desc_s.sib2.sib2conf.q_Hyst;
    // neighbouring cell rank:
    double R_n;

    std::vector<int> ranking;
    
    // reselection criterion itself:
    // R_n > R_s, where:
    // R_s = Qmeas_s + Qhyst
    // R_n = Qmeas_n - Qoffset  # (Qoffset = 0)

    for (auto &item : m_cellDesc) {
        if (cellInfo.cellId == item.first)
            continue;

        R_n = m_base->config->rssi[item.second.sib1.nci].RSRP - 156.0;
        //std::cout << "[cell " << item.first << "] " << "R_n: " << R_n << "\n";
        if (R_n > R_s)
    ...
    std::sort(ranking.begin(), ranking.end(), [this](int a, int b) {
        auto &cellA = m_base->config->rssi[a];
        auto &cellB = m_base->config->rssi[b];
        return cellB.RSRP < cellA.RSRP;
    });

    auto &selectedId = ranking[0];
    ...
    if (cellInfo.cellId != 0 && cellInfo.cellId != lastCellId)
        m_logger->info("Reselected cell plmn[%s] tac[%d] category[%s]", ToJson(cellInfo.plmn).str().c_str(), cellInfo.tac,
...
bool UeRrcTask::isCellSuitable(int id) {

    UeCellDesc cell_desc = m_cellDesc[id];
    RSSI ue_rssi = m_base->config->rssi[cell_desc.sib1.nci];    
    // Srxlev = Qrxlevmeas - (Qrxlevmin + QrxlevminOffset) – Pcompensation
    double s_RxLev = (ue_rssi.RSRP - 156.0) - (cell_desc.sib1.sib1conf.q_RxLevMin * 2.0 + *cell_desc.sib1.sib1conf.q_RxLevMinOffset);
    
    // Squal = Qqualmeas – (Qqualmin + QqualminOffset)
    double s_Qual = (ue_rssi.RSRQ - 87.0) / 2.0 - (*cell_desc.sib1.sib1conf.q_QualMin + *cell_desc.sib1.sib1conf.q_QualMinOffset);

    if (s_RxLev > 0 && s_Qual > 0)
        return true;
    return false;
}
...
bool UeRrcTask::lookForSuitableCell(ActiveCellInfo &cellInfo, CellSelectionReport &report)
{
    ...
    for (auto &item : m_cellDesc)
    {
        ...
        if (!isCellSuitable(item.first)) {
            report.notSuitableCells++;
            continue;
        }
