...
static OrderedMap<std::string, CmdEntry> g_ueCmdEntries = {
    ...
    {"register", {"Register UE i.e. allow transition to state RRC_CONNECTED", "", DefaultDesc, false}},
    {"set-rsrp", {"modify rsrp", "", DefaultDesc, false}},
    {"set-rsrq", {"modify rsrq", "", DefaultDesc, false}}
};
                                                    ...
                                                    std::string &error)
{
    ...
    else if (subCmd == "coverage")
    {
        ...
        return std::make_unique<UeCliCommand>(UeCliCommand::REGISTER);
    }
    else if (subCmd == "set-rsrp") {
        auto cmd = std::make_unique<UeCliCommand>(UeCliCommand::SET_RSRP);
        std::string x_string = options.getPositional(0);
        utils::TryParseInt(x_string, cmd->x);
        return cmd;
    }
    else if (subCmd == "set-rsrq") {
        auto cmd = std::make_unique<UeCliCommand>(UeCliCommand::SET_RSRQ);
        std::string x_string = options.getPositional(0);
        utils::TryParseInt(x_string, cmd->x);
        return cmd;
    }
